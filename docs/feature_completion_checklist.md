...
| Date-based project history | partial | Project sections and the project detail sheet show local-date grouped record history/table rows for notes, numeric logs, image proof, action context, and habit-generated records; a full standalone history page is postponed. |
| Project statistics foundation | partial | Project detail shows project identity, parent goal context, task-derived progress, subproject/action/completed counts, habit-generated record count, record count, active days, current-month records, photo count, record composition bars, current-month heatmap, and scan-friendly numeric totals grouped by unit; chart packages and yearly project heatmap are postponed. |
